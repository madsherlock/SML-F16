...
loadSinglePersonsData <- function(DPI,groupNr,groupMemberNr){
  ...
  #color grid to show how well the different ciffer boxes are segmented
  for(pages in 1:5)
  {
    for(box in 1:2)
    {
  ...
  #Print the training data for digit "0" to see whether it is reasonable.
  print( trainingDigit[[1]] )
   
  #return the tr #load the scaned images
  ciffers <- list(readPNG(paste(c("../../SML-database/2016/group",groupNr,"/member",groupMemberNr,"/Ciphers",DPI,"-0.png"), collapse = "")),
                  readPNG(paste(c("../../SML-database/2016/group",groupNr,"/member",groupMemberNr,"/Ciphers",DPI,"-1.png"), collapse = "")),
                  readPNG(paste(c("../../SML-database/2016/group",groupNr,"/member",groupMemberNr,"/Ciphers",DPI,"-2.png"), collapse = "")),
                  readPNG(paste(c("../../SML-database/2016/group",groupNr,"/member",groupMemberNr,"/Ciphers",DPI,"-3.png"), collapse = "")),
  ...
  #load the corner values
  corners <- read.csv(paste(c("../../SML-database/2016/group",groupNr,"/member",groupMemberNr,"/Corners.txt"), collapse = ""))
  corners <- trunc(corners*DPI/300)aining data for all digits.
  #This return value is a vector of tables. Each table in this vector represents
  #a single type of digit, so the first table is only 0's, the second is 1's and so on.
  #In the tables the rows represent the individual handwritten digits.
  #The columns represents the pixel values.
...
#in this example the code load the data and put it into a data
#structure which is better suited for R classification
#-------------------------------------------------------------
#Example for loading single person data:

#Specify the person and the DPI
